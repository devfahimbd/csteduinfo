<?php
/**
 * CST Department Website - Teacher Detail Page
...
if (empty($slug)) {
    header('Location: ' . SITE_URL . '/faculty.php');
    exit;
}

// Extract ID from end of slug (pattern: name-slug-ID)
$parts = explode('-', $slug);
$id = intval(end($parts));
...
$teacher = null;
try {
    $stmt = $pdo->prepare("SELECT t.*, c.name AS category_name FROM teachers t LEFT JOIN categories c ON t.category_id = c.id WHERE t.id = ? AND t.status = 1");
    $stmt->execute([$id]);
    $teacher = $stmt->fetch();
} catch (Exception $e) {
    error_log("Teacher fetch error: " . $e->getMessage());
    $teacher = null;
}

// ─── Page Title ───
$pageTitle = ($teacher ? clean($teacher['name']) . ' - ' : 'শিক্ষকমণ্ডলী - ') . SITE_NAME;
...
<head>
    ...
    <style>
        ...
        .teacher-detail-right .designation {
            font-size: 16px;
            font-weight: 600;
            color: #2563EB;
            margin-bottom: 4px;
        ...
        .teacher-detail-right .qualification {
            font-size: 14px;
            color: #64748B;
            margin-bottom: 20px;
        }

        .teacher-detail-right .teacher-bio {
            font-size: 15px;
            color: #1E293B;
            line-height: 1.85;
            margin-bottom: 28px;
        ...
        .teacher-contact-list li .contact-icon {
            width: 36px;
            height: 36px;
            border-radius: 8px;
            background: #EFF6FF;
        ...
        .teacher-contact-list li .contact-icon svg {
            width: 16px;
            height: 16px;
        }
...
<section class="page-banner">
    <div class="container">
        <h1><?php echo clean($teacher['name']); ?></h1>
        <div class="breadcrumb">
            ...
            <a href="<?php echo SITE_URL; ?>/faculty.php">শিক্ষকমণ্ডলী</a>
            <span class="separator">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
            </span>
            <span><?php echo clean($teacher['name']); ?></span>
        </div>
    </div>
</section>
<?php endif; ?>
...
<section class="detail-page">
    <div class="container">
            ...
            <div class="teacher-detail-right">
                ...
                <?php if (!empty($teacher['bio'])): ?>
                    <div class="teacher-bio">
                        <?php echo nl2br(clean($teacher['bio'])); ?>
                    </div>
                <?php else: ?>
                    <div style="margin-bottom:28px;padding-bottom:24px;border-bottom:1px solid #F1F5F9;"></div>
                <?php endif; ?>

                <!-- Contact Info -->
                <?php if (!empty($teacher['email']) || !empty($teacher['phone'])): ?>
                <ul class="teacher-contact-list">
                    <?php if (!empty($teacher['email'])): ?>
                    <li>
                        <span class="contact-icon">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                        </span>
                        <a href="mailto:<?php echo clean($teacher['email']); ?>"><?php echo clean($teacher['email']); ?></a>
                    </li>
                    <?php endif; ?>
                    ...
                    <li>
                        ...
                        <a href="tel:<?php echo clean($teacher['phone']); ?>"><?php echo clean($teacher['phone']); ?></a>
                    </li>
                    <?php endif; ?>
                </ul>
                <?php endif; ?>
                ...
                <!-- Social Links -->
                <?php if (!empty($teacher['facebook']) || !empty($teacher['linkedin'])): ?>
        ...
        <div class="back-link-wrapper" style="margin-top:32px;">
            <a href="<?php echo SITE_URL; ?>/faculty.php" class="back-link">
...
<section class="not-found-section">
    ...
    <h2>শিক্ষক খুঁজে পাওয়া যায়নি</h2>
    <p>আপনি যে শিক্ষকটি খুঁজছেন সেটি বিদ্যমান নেই বা সরানো হয়েছে।</p>
    <a href="<?php echo SITE_URL; ?>/faculty.php" class="btn btn-outline">
