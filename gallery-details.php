<?php
/**
 * CST Department Website - Gallery Detail Page
...
$galleryItem = null;
try {
    $stmt = $pdo->prepare("SELECT g.*, c.name AS category_name FROM gallery g LEFT JOIN categories c ON g.category_id = c.id WHERE g.slug = ? AND g.status = 1");
    $stmt->execute([$slug]);
...
$catName = '';
if ($galleryItem) {
    $catName = !empty($galleryItem['category_name']) ? $galleryItem['category_name'] : getCategoryName($pdo, $galleryItem['category_id']);
}

// ─── Fetch Previous / Next Gallery Items ───
$prevItem = null;
$nextItem = null;

if ($galleryItem) {
    try {
        // Previous item (lower id)
        $stmtPrev = $pdo->prepare("SELECT id, title, slug FROM gallery WHERE id < ? AND status = 1 ORDER BY id DESC LIMIT 1");
        $stmtPrev->execute([$galleryItem['id']]);
        $prevItem = $stmtPrev->fetch();

        // Next item (higher id)
        $stmtNext = $pdo->prepare("SELECT id, title, slug FROM gallery WHERE id > ? AND status = 1 ORDER BY id ASC LIMIT 1");
        $stmtNext->execute([$galleryItem['id']]);
        $nextItem = $stmtNext->fetch();
    } catch (Exception $e) {
        error_log("Gallery prev/next fetch error: " . $e->getMessage());
        $prevItem = null;
        $nextItem = null;
    }
}
...
// ─── Page Title ───
$pageTitle = ($galleryItem ? clean($galleryItem['title']) . ' - ' : 'গ্যালারি - ') . SITE_NAME;
...
$footerText    = siteSetting('footer_text', '&copy; ' . date('Y') . ' সিএসটি বিভাগ। সর্বস্বত্ব সংরক্ষিত।');
?>
<!DOCTYPE html>
<html lang="bn">
<head>
    ...
    <meta name="description" content="<?php echo clean($pageTitle); ?>">
    ...
    <style>
        ...
        .gallery-detail-meta .meta-category {
            background: #DBEAFE;
            color: #2563EB;
            padding: 3px 12px;
            border-radius: 20px;
        ...
        .not-found-section lottie-player {
            margin-bottom: 0px;
        }
...
<section class="page-banner">
    <div class="container">
        <h1><?php echo clean($galleryItem['title']); ?></h1>
        <div class="breadcrumb">
            ...
            <a href="<?php echo SITE_URL; ?>/gallery.php">গ্যালারি</a>
            <span class="separator">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
            </span>
            <span><?php echo clean($galleryItem['title']); ?></span>
        </div>
    </div>
</section>
<?php endif; ?>
...
<section class="detail-page">
    <div class="container">
        <div class="gallery-detail-content">
            ...
            <?php else: ?>
                <div style="width:100%;height:320px;background:#F1F5F9;display:flex;align-items:center;justify-content:center;">
                    <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" width="64" height="64">
            ...
            <div class="gallery-detail-info">
                ...
                <div class="gallery-detail-meta">
                    ...
                    <span>
                        ...
                        <?php echo date('d M, Y', strtotime($galleryItem['created_at'])); ?>
                    </span>
                </div>

                <?php if (!empty($galleryItem['description'])): ?>
                    <div class="gallery-detail-description">
                        <?php echo nl2br(clean($galleryItem['description'])); ?>
                    </div>
                <?php endif; ?>
                ...
                <div class="gallery-nav">
                    <?php if ($prevItem): ?>
                        <a href="<?php echo SITE_URL; ?>/gallery-details.php?slug=<?php echo clean($prevItem['slug']); ?>">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>
                            <?php echo clean($prevItem['title']); ?>
                        </a>
                    <?php else: ?>
                        <span class="gallery-nav a nav-disabled" style="display:inline-flex;align-items:center;gap:8px;padding:10px 20px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:10px;font-size:14px;font-weight:500;color:#1E293B;">
                    ...
                    <?php if ($nextItem): ?>
                        <a href="<?php echo SITE_URL; ?>/gallery-details.php?slug=<?php echo clean($nextItem['slug']); ?>">
                            <?php echo clean($nextItem['title']); ?>
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
                        </a>
                    <?php else: ?>
                        <span style="display:inline-flex;align-items:center;gap:8px;padding:10px 20px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:10px;font-size:14px;font-weight:500;color:#1E293B;">
                            পরবর্তী
        ...
        <!-- Back Link -->
        <div class="back-link-wrapper" style="margin-top:32px;">
            <a href="<?php echo SITE_URL; ?>/gallery.php" class="back-link">
...
<section class="not-found-section">
    <lottie-player
        ...
        background="transparent"
        speed="1"
        style="width: 280px; height: 280px; display: block; margin: 0 auto; margin-bottom: 0;"
        loop
        autoplay>
    </lottie-player>
    <h2>গ্যালারি আইটেম খুঁজে পাওয়া যায়নি</h2>
    <p>আপনি যে গ্যালারি আইটেমটি খুঁজছেন সেটি বিদ্যমান নেই বা সরানো হয়েছে।</p>
    <a href="<?php echo SITE_URL; ?>/gallery.php" class="btn btn-outline">
    ...
    <div class="container">
        <div class="footer-grid">
            ...
            <div class="footer-col">
                ...
                <p><?php echo clean($siteDesc) ?: 'কম্পিউটার সায়েন্স অ্যান্ড টেকনোলজি বিভাগের অফিসিয়াল ওয়েবসাইট। একাডেমিক উৎকর্ষ এবং উদ্ভাবনার প্রতিশ্রুতিবদ্ধ।'; ?></p>
            ...
            <div class="footer-col">
                ...
                <ul class="footer-links">
                    <li><a href="<?php echo SITE_URL; ?>"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg> হোম</a></li>
                    <li><a href="<?php echo SITE_URL; ?>/about.php"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg> সম্পর্কে</a></li>
                    <li><a href="<?php echo SITE_URL; ?>/faculty.php"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg> শিক্ষকমণ্ডলী</a></li>
                    <li><a href="<?php echo SITE_URL; ?>/notice.php"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg> নোটিশ</a></li>
                    <li><a href="<?php echo SITE_URL; ?>/gallery.php"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg> গ্যালারি</a></li>
                    <li><a href="<?php echo SITE_URL; ?>/contact.php"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg> যোগাযোগ</a></li>
                </ul>
            </div>
            ...
            <div class="footer-col">
                <h4>রিসোর্স</h4>
                <ul class="footer-links">
                    <li><a href="<?php echo SITE_URL; ?>/resources.php"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg> লেকচার নোট</a></li>
                    <li><a href="<?php echo SITE_URL; ?>/resources.php"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg> ই-বুক</a></li>
                    <li><a href="<?php echo SITE_URL; ?>/resources.php"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg> সফটওয়্যার</a></li>
                    <li><a href="<?php echo SITE_URL; ?>/notice.php"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg> পরীক্ষার সময়সূচি</a></li>
                    <li><a href="<?php echo SITE_URL; ?>/notice.php"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg> একাডেমিক ক্যালেন্ডার</a></li>
                </ul>
            </div>
