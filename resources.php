<?php
/**
 * পলিটেকনিক শিক্ষা বাংলাদেশ - Resources Page
 * Core PHP + MySQL with PDO
 */

require_once 'includes/config.php';

$pageTitle = 'Resources & Materials - ' . SITE_NAME;
...
$resourceCategories = [];
try {
    $stmt = safeQuery($pdo, "SELECT * FROM categories WHERE type = 'resource' AND status = 1 ORDER BY name ASC");
    if ($stmt) {
...
// ─── Fetch All Active Resources with Category ───
$resources = [];
try {
    $stmt = safeQuery($pdo, "SELECT r.*, c.name AS category_name FROM resources r LEFT JOIN categories c ON r.category_id = c.id WHERE r.status = 1 ORDER BY r.created_at DESC");
    if ($stmt) {
...
<section class="section">
    <div class="container">
        ...
        <div class="section-header">
            ...
            <h2 class="section-title">রিসোর্স ও উপকরণ</h2>
            <p class="section-desc">ভাগ করা লেকচার নোট, ই-বুক, সফটওয়্যার টুল এবং অন্যান্য একাডেমিক রিসোর্সে অ্যাক্সেস পান।</p>
        </div>
        ...
        <?php if (!empty($resources)): ?>
            ...
            <div class="filter-tabs" id="resource-filter-tabs" style="margin-bottom:30px;">
            ...
            <div class="resource-list" id="resource-list">
                <?php foreach ($resources as $resource):
                    ...
                    if ($hasFile) {
                        $linkUrl  = SITE_URL . '/download.php?type=resource&id=' . (int) $resource['id'];
                        $btnLabel = 'ডাউনলোড';
                        $btnIcon  = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>';
                    } elseif ($hasUrl) {
                        $linkUrl  = clean($resource['external_url']);
                        $btnLabel = 'দেখুন';
                        $btnIcon  = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>';
                    } else {
                        $linkUrl  = '#';
                        $btnLabel = 'দেখুন';
                        $btnIcon  = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>';
                    }
                ?>
                    <div class="resource-card" data-category="<?php echo clean($catName); ?>">
                        ...
                        <div class="resource-info">
                            <h3>
                                <?php if ($hasUrl): ?>
                                    <a href="<?php echo $linkUrl; ?>" target="_blank" rel="noopener"><?php echo clean($resource['title']); ?></a>
                                <?php else: ?>
                                    <a href="<?php echo $linkUrl; ?>"><?php echo clean($resource['title']); ?></a>
                                <?php endif; ?>
                            </h3>
                            <?php if ($catName): ?>
                                <span class="category">
                            ...
                            <div class="resource-meta">
                                ...
                                <span>
                                    ...
                                    <?php echo date('h:i A', strtotime($resource['created_at'])); ?>
                                </span>
                            </div>
                        </div>
                        <a href="<?php echo $linkUrl; ?>" class="btn-sm btn-primary" <?php echo ($hasUrl ? 'target="_blank" rel="noopener"' : ''); ?>>
                            <?php echo $btnIcon; ?>
                            <?php echo $btnLabel; ?>
                        </a>
                    </div>
        ...
        <?php else: ?>
            ...
            <div class="empty-state" style="margin-top: -10px;">
                ...
                <h3>কোনো রিসোর্স উপলব্ধ নেই</h3>
                <p>বর্তমানে কোনো রিসোর্স প্রকাশিত হয়নি। অনুগ্রহ করে পরে আবার দেখুন।</p>
            </div>
    ...
    <div class="container">
        <div class="footer-grid">
            ...
            <div class="footer-col">
                ...
                <p><?php echo clean($siteDesc) ?: 'পলিটেকনিক শিক্ষার অফিসিয়াল ওয়েবসাইট। একাডেমিক উৎকর্ষ ও উদ্ভাবনের প্রতিশ্রুতি।'; ?></p>
...
(function() {
    'use strict';
    var filterTabs = document.querySelectorAll('#resource-filter-tabs .filter-tab');
    var resourceCards = document.querySelectorAll('#resource-list .resource-card');

    if (filterTabs.length && resourceCards.length) {
