...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        try {
            ...
            $slug = generateSlug($title, 'resources', 'slug', $id);
            ...
            if (!empty($_FILES['file']['name'])) {
                ...
                if ($result['success']) {
                    if (!empty($resource['file'])) deleteFile($resource['file']);
                    $file = $result['path'];
                }
            }

            $stmt = $pdo->prepare("UPDATE resources SET title=?, slug=?, type=?, semester=?, category_id=?, description=?, file=?, status=? WHERE id=?");
            $stmt->execute([$title, $slug, $type, $semester, $categoryId, $description, $file, $status, $id]);

            setFlash('success', 'Resource updated successfully.');
            header('Location: index.php');
            exit;
        } catch (PDOException $e) {
            setFlash('error', 'Failed to update resource.');
        }
    }
}
...
<div class="admin-layout">
    ...
    <main class="admin-content">
        <header class="admin-header">
            <div class="breadcrumb"><a href="../dashboard.php">Dashboard</a> &nbsp;/&nbsp; <a href="index.php">Resources</a> &nbsp;/&nbsp; <strong>Edit Resource</strong></div>
                ...
                <div class="panel">
                    ...
                    <div class="panel-body">
                        <div class="form-group">
                            <label>Title *</label>
                            <input type="text" name="title" required value="<?php echo htmlspecialchars($resource['title']); ?>">
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label>Type *</label>
                                <select name="type">
                                    <?php foreach ($types as $key => $label): ?>
                                        <option value="<?php echo $key; ?>" <?php echo $resource['type'] === $key ? 'selected' : ''; ?>><?php echo $label; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select name="semester">
                                    ...
                                    <?php foreach ($semesters as $sem): ?>
                                        <option value="<?php echo $sem; ?>" <?php echo $resource['semester'] === $sem ? 'selected' : ''; ?>><?php echo $sem; ?> Semester</option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="category_id">
                                    ...
                                    <?php foreach ($categories as $cat): ?>
                                        <option value="<?php echo $cat['id']; ?>" <?php echo ($resource['category_id'] == $cat['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($cat['name']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="description" rows="3"><?php echo htmlspecialchars($resource['description'] ?? ''); ?></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="panel">
                    ...
                    <div class="panel-body">
                        <div class="form-group">
                            ...
                            <p class="hint">Leave empty to keep current file.</p>
                            <?php if (!empty($resource['file'])): ?>
                                <div class="current-file">Current: <?php echo htmlspecialchars(basename($resource['file'])); ?></div>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
