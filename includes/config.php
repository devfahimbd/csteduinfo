...
define('MAX_FILE_SIZE', 50 * 1024 * 1024); // 50MB for PDF
...
function getSettings() {
    global $site_settings;
    if (!defined('SETTINGS')) define('SETTINGS', $site_settings);
}
...
function getFlash() { if (isset($_SESSION['flash'])) { $flash = $_SESSION['flash']; unset($_SESSION['flash']); return $flash; } return null; }
...
function getCategoryName($pdo, $id) {
    ...
    try { $stmt = $pdo->prepare("SELECT name FROM categories WHERE id = ?"); $stmt->execute([$id]); $row = $stmt->fetch(); return $row ? $row['name'] : 'Uncategorized'; } catch (Exception $e) { return 'Uncategorized'; }
}

function safeQuery($pdo, $sql, $params = []) {
    try { $stmt = $pdo->prepare($sql); $stmt->execute($params); return $stmt; } catch (Exception $e) { error_log("Query Error: " . $e->getMessage()); return false; }
}
...
function uploadFile($file, $dir, $allowed = ['jpg','jpeg','png','gif','webp','pdf','doc','docx','ppt','pptx','xls','xlsx','zip']) {
    ...
    if (move_uploaded_file($file['tmp_name'], $targetPath)) return $dir . '/' . $newName;
    return null;
}

function deleteFile($filepath) { if ($filepath && file_exists(UPLOAD_PATH . '/' . $filepath)) unlink(UPLOAD_PATH . '/' . $filepath); }
